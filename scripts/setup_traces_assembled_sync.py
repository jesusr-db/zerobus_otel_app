...
# MAGIC %md
# MAGIC ## Create Traces Assembled Synced Table

# COMMAND ----------

table_name = f"{catalog_name}.{schema_name}.traces_assembled_synced"
source_table = f"{catalog_name}.{schema_name}.traces_assembled"
...
try:
        ...
        if pipeline_id:
            try:
                ...
                if "not found" in str(pipeline_error).lower():
                    print(f"   Pipeline {pipeline_id} was deleted - need to recreate synced table")
                    needs_recreation = True
                else:
                    print(f"   Could not verify pipeline: {pipeline_error}")
    else:
    ...
    if "not found" in str(e).lower() or "does not exist" in str(e).lower():
        ...
        print(f"\nError checking existing table: {e}")
        raise
...
if needs_recreation and existing_table:
    print(f"\nDeleting orphaned synced table: {table_name}")
    try:
...
print(f"   Database: {database_instance_name}.{schema_name}.traces_assembled_synced")
print(f"\n   To query from Lakebase:")
print(f"   SELECT * FROM {schema_name}.traces_assembled_synced WHERE trace_id = 'xyz';")
