...
export function MetricsView() {
  ...
  const getAllMetrics = () => {
    ...
    if (kpis.metrics_by_type.histogram) {
      Object.entries(kpis.metrics_by_type.histogram).forEach(
        ([name, metric]) => {
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-bold">
                              {metric.data.statistics.max.value}
                            </span>
                            <TrendIcon
                              trend={metric.data.statistics.max.trend}
                            />
                          </div>
                        </div>
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-bold">
                              {metric.data.statistics.avg.value}
                            </span>
                            <TrendIcon
                              trend={metric.data.statistics.avg.trend}
                            />
                          </div>
                        </div>
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-bold">
                              {metric.data.statistics.min.value}
                            </span>
                            <TrendIcon
                              trend={metric.data.statistics.min.trend}
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4">
                        <MetricMultiLineChart
                          series={[
                            {
                              name: "Max",
                              data: metric.data.statistics.max.timeseries || [],
                              color: "hsl(0, 84%, 60%)",
                            },
                            {
                              name: "Avg",
                              data: metric.data.statistics.avg.timeseries || [],
                              color: "hsl(220, 70%, 60%)",
                            },
                            {
                              name: "Min",
                              data: metric.data.statistics.min.timeseries || [],
                              color: "hsl(160, 60%, 45%)",
                            },
                          ]}
                          height={280}
                      ...
                      <div className="flex items-center justify-between p-4 bg-muted/20 rounded-lg">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-2xl font-bold">
                            {metric.data.sum.total.value.toLocaleString()}
                          </span>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xl font-bold">
                              {metric.data.sum.rate.value}
                              {metric.data.sum.rate.unit}
                            </span>
                            <TrendIcon trend={metric.data.sum.rate.trend} />
                          </div>
                        </div>
                        {metric.data.sum.rate.timeseries &&
                          metric.data.sum.rate.timeseries.length > 0 && (
                            <MetricBarChart
