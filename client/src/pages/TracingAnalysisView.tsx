...
export function TracingAnalysisView() {
  ...
  const filteredTraces = selectedService
    ? allTraces?.filter((trace) =>
        trace.services_involved.includes(selectedService),
      )
    : allTraces;
  ...
  const buildSpanHierarchy = (spans: SpanWaterfall[]) => {
    const spanMap = new Map(
      spans.map((s) => [
        s.span_id,
        { ...s, children: [] as SpanWaterfall[], depth: 0 },
      ]),
    );
    const roots: any[] = [];
  ...
  const renderWaterfallSpan = (
    ...
    return (
      <div key={span.span_id} className="mb-1 relative">
        ...
        >
          {span.depth > 0 && <span className="text-muted-foreground">└─</span>}
          <span
            className="w-32 truncate font-medium text-foreground"
                  ...
                  <div className="space-y-1">
                    ...
                    {span.parent_span_id && (
                      <div className="flex justify-between gap-4">
                        ...
                        <span className="font-mono text-xs">
                          {span.parent_span_id.substring(0, 16)}...
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between gap-4">
                      ...
                      <span className="font-mono">
                        {span.start_offset_ms.toFixed(2)}ms
                      </span>
                    </div>
                    <div className="flex justify-between gap-4">
      ...
      <div className="flex h-full flex-col">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            Visualize trace spans in waterfall format with parent-child
            relationships
          </p>
        </div>
        ...
        {tracesError && (
          <div className="flex h-full items-center justify-center">
            <div className="text-red-500">
              Error loading traces: {tracesError.message}
            </div>
          </div>
        )}
        ...
        <div className="flex gap-6 flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {selectedTraceId && waterfallError && (
              <div className="flex items-center justify-center h-full">
                <div className="text-red-500">
                  Error loading waterfall: {waterfallError.message}
                </div>
              </div>
            )}
              ...
              waterfallData && (
                <div>
                  <div className="mb-6">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      <div>
                        Trace ID:{" "}
                        <span className="font-mono">
                          {waterfallData.trace_id}
                        </span>
                      </div>
                      <div>
                        Start:{" "}
                        {new Date(waterfallData.trace_start).toLocaleString()}
                      </div>
                      <div>
                        Total Duration:{" "}
                        {waterfallData.total_duration_ms.toFixed(2)}ms
                      </div>
                  ...
                  <div className="rounded-lg border border-border bg-card p-4">
                    ...
                    {(() => {
                      const hierarchicalSpans = buildSpanHierarchy(
                        waterfallData.spans,
                      );
                      const servicesList = Array.from(
                        new Set(waterfallData.spans.map((s) => s.service_name)),
                      ).sort();
                        ...
                        <>
                          <div className="mb-4 flex flex-wrap gap-2">
                            {servicesList.map((service) => (
                              ...
                              >
                                <div
                                  className={`w-3 h-3 rounded ${getServiceColor(service, servicesList)}`}
                                />
                          ...
                          {hierarchicalSpans.map((span) =>
                            renderWaterfallSpan(
                              span,
                              waterfallData.total_duration_ms,
                              servicesList,
                            ),
                          )}
                        </>
