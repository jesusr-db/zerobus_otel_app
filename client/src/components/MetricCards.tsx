  ...
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        {value.timeseries && value.timeseries.length > 0 && (
          <Sparkline data={value.timeseries} />
        )}
      </div>
    </div>
  );
      ...
      <CardContent>
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div
              className={`text-2xl font-bold ${getHealthColor(current.value)}`}
            >
              {current.value}
              {current.unit || "%"}
            </div>
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {current.timeseries && current.timeseries.length > 0 && (
              <Sparkline
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="font-medium">
              {metric.sum.total.value.toLocaleString()}
            </span>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="font-medium">
              {metric.sum.rate.value}
              {metric.sum.rate.unit}
            </span>
            <TrendIcon trend={metric.sum.rate.trend} />
            {metric.sum.rate.timeseries &&
              metric.sum.rate.timeseries.length > 0 && (
                <Sparkline data={metric.sum.rate.timeseries} />
              )}
          </div>
        </div>
      </CardContent>
